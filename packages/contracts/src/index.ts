...
for(const name of ['id','site_id','package_id','batch_id','created_by','cashier_id','reservation_id','sale_id','voucher_id','grant_id','reviewed_by','actor_id','entity_id','name','description','label','role','display_name','code_mask','package_name','inventory_state','policy','receipt_number','cashier_name','currency','payment_method','client_mac','state','action','code','message','status','provider','network','payment_reference','provider_reference','failure_reason','session_id','session_state','upload_bytes','download_bytes'])fields[name]=T.Optional(T.String());
for(const name of ['created_at','updated_at','sold_at','expires_at','activated_at','proposed_expires_at','deadline','reversal_reason','reason','review_note','customer_name','customer_phone','notes','last_seen_at','stopped_at','terminate_cause','paid_at'])fields[name]=T.Optional(nullableString);
