...
export async function buildApp(options:{adapter?:Adapter;verifyToken?:(token:string)=>Promise<string|null>;logger?:boolean}={}){
 ...
 route('GET','/sales',undefined,S.List,async(r:any)=>paged('select s.* from wifi.manual_sales s where ($1::boolean or cashier_id=$2) and receipt_number ilike $3',[r.staff.role==='ADMIN',r.staff.id,'%'+(r.query.q||'')+'%'],r.query),false,{schema:{querystring:S.Paging}});
