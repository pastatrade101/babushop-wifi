...
const links=[['/','Overview','overview'],['/sell','Sell internet','plus'],['/vouchers','Vouchers','ticket'],['/voucher-batches','Voucher batches','batch'],['/packages','Packages','package'],['/sales','Sales','sales'],['/access-grants','Access grants','grants'],['/sessions','Wi-Fi sessions','wifi'],['/reports','Reports','reports'],['/staff','Staff','staff'],['/settings','Network setup','settings']];
...
<div class="workspace">
 <aside class="sidebar">
  ...
  <div id="workspace-navigation" class="sidebar-body" class:open={menuOpen}>
   <p class="sidebar-label">WORKSPACE</p><nav aria-label="Main navigation">{#each links as [url,label,icon]}{#if data.staff.role==='ADMIN'||cashier.includes(url)}<a href={url} class:current={page.url.pathname===url} aria-current={page.url.pathname===url?'page':undefined} onclick={()=>menuOpen=false}><Icon name={icon} size={18}/>{label}</a>{/if}{/each}</nav>
